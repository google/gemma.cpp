...
struct ConversationData {
  ...
  // Constructor to initialize kv_cache (requires KVCache definition or forward declaration)
  ConversationData(ModelConfig model_config, size_t prefill_tbatch_size);
};
...
class GemmaContext {
 private:
  GemmaContext(LoaderArgs loader, InferenceArgs inference_args,
    ThreadingArgs threading_args, int max_length);

 public:
  static GemmaContext* Create(const char* tokenizer_path, const char* model_type,
  ...
  int GenerateMultimodal(const char* prompt_string, const void* image_data,
                         int image_width, int image_height, // Added dimensions
  ...
  void ResetConversation() {
    if (active_conversation) {
      ...
      LogDebug("Cannot reset conversation: model or active_conversation is null");
    }
  }
  ...
  bool SwitchConversation(const char* conversation_name) {
    ...
    active_conversation = it->second; // Assign the shared_ptr
    return true;
  }
 ...
 private:
  ...
  std::unordered_map<std::string, std::shared_ptr<ConversationData>> conversation_cache;

  // Buffers (potentially could be moved into ConversationData if needed per-conversation)
