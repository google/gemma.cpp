...
struct Activations {
  Activations(const RuntimeConfig& runtime_config, const ModelConfig& config,
      ...
      : layer_config(config.layer_configs[0]),
        ...
        mla_o_group(MatFactory("mla_o_group",
                               mla_dims.o_mid_dim > 0 ? batch_size : 0,
                               mla_dims.o_mid_dim > 0
                                   ? MaxOLoraRank(config)
                                   : 0,
                               ctx.allocator)),
        ...
        hc_streams(MatFactory("hc_streams",
                              config.hc_mult > 1 ? batch_size : 0,
        ...
        mla_inv_timescale_c(CreateYarnInvTimescale(
            ...
            config.compress_rope_theta > 0.0f ? config.compress_rope_theta
                                              : config.rope_theta,
            config.yarn_factor, config.yarn_orig_seq_len,
