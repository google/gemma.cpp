...
#ifndef BENCHMARK_BLOCK_SIZE
#define BENCHMARK_BLOCK_SIZE 128
#endif
...
template <int kNumQueries, typename T_IN, typename DF,
          typename VF = hn::Vec<DF>>
HWY_INLINE void QDotKTilexUpTo8MatrixAccumulation(
    DF df, const T_IN* HWY_RESTRICT q_group, const T_IN* tile_base,
    size_t current_pos, size_t qkv_dim, VF& C00, VF& C01, VF& C02, VF& C03,
    VF& C10, VF& C11, VF& C12, VF& C13, VF& C20, VF& C21, VF& C22, VF& C23,
    VF& C30, VF& C31, VF& C32, VF& C33) {
  using D_ACC = hwy::If<IsInt8<T_IN>(), hn::FixedTag<int32_t, 4>, DF>;
  const D_ACC d_acc;
  using VecAcc = hn::Vec<D_ACC>;

  VecAcc acc00 = hn::Zero(d_acc), acc01 = hn::Zero(d_acc),
         acc02 = hn::Zero(d_acc), acc03 = hn::Zero(d_acc);
  VecAcc acc10 = hn::Zero(d_acc), acc11 = hn::Zero(d_acc),
         acc12 = hn::Zero(d_acc), acc13 = hn::Zero(d_acc);
  VecAcc acc20 = hn::Zero(d_acc), acc21 = hn::Zero(d_acc),
         acc22 = hn::Zero(d_acc), acc23 = hn::Zero(d_acc);
  VecAcc acc30 = hn::Zero(d_acc), acc31 = hn::Zero(d_acc),
         acc32 = hn::Zero(d_acc), acc33 = hn::Zero(d_acc);

  using D_INPUT = hn::Repartition<T_IN, DF>;
  const D_INPUT d_input;
  using VecInput = hn::Vec<D_INPUT>;

  HWY_LANES_CONSTEXPR size_t step_size = hn::Lanes(d_input);
  HWY_LANES_CONSTEXPR size_t ch_step = 4;  // Always 4 for 8x4 layout

  size_t g0 = (current_pos / 8) % 4;
  const T_IN* k_ptr0;
  const T_IN* k_ptr1;
  const T_IN* k_ptr2;
  const T_IN* k_ptr3;
  ...
  if (step_size == 32) {
    ...
    k_ptr0 = tile_base + g0 * 32;
    k_ptr1 = tile_base + (g0 + 1) * 32;
    k_ptr2 = nullptr;
    k_ptr3 = nullptr;
  } else {  // step_size == 8
    k_ptr0 = tile_base + g0 * 32;
  ...
  for (size_t ch_base = 0; ch_base < qkv_dim; ch_base += ch_step) {
    ...
    if (step_size == 32) {  // 512-bit native path
      B0 = hn::LoadU(d_input, k_ptr0 + ch_base_k);
      B1 = hn::LoadU(d_input, k_ptr1 + ch_base_k);
      B2 = hn::LoadU(d_input, k_ptr2 + ch_base_k);
      B3 = hn::LoadU(d_input, k_ptr3 + ch_base_k);
    } else if (step_size == 16) {  // 256-bit fallback path
      B0 = hn::LoadU(d_input, k_ptr0 + ch_base_k);
      B1 = hn::LoadU(d_input, k_ptr0 + ch_base_k + 16);  // Group 0, second half
      B2 = hn::LoadU(d_input, k_ptr1 + ch_base_k);
      B3 = hn::LoadU(d_input, k_ptr1 + ch_base_k + 16);  // Group 1, second half
    } else if (step_size == 8) {  // 128-bit fallback path
      B0 = hn::LoadU(d_input, k_ptr0 + ch_base_k);
      B1 = hn::LoadU(d_input,
                     k_ptr0 + ch_base_k + 8);  // Group 0, second quarter
      B2 = hn::LoadU(d_input,
                     k_ptr0 + ch_base_k + 16);  // Group 0, third quarter
      B3 = hn::LoadU(d_input,
                     k_ptr0 + ch_base_k + 24);  // Group 0, fourth quarter
    }

    if constexpr (kNumQueries >= 1) {
      const auto A0 = LoadAndDuplicateQueries(d_input, q_group + ch_base_q);
      acc00 = PerBlock2x2MatMulMaybeEmulate(d_acc, A0, B0, acc00);
      acc01 = PerBlock2x2MatMulMaybeEmulate(d_acc, A0, B1, acc01);
      acc02 = PerBlock2x2MatMulMaybeEmulate(d_acc, A0, B2, acc02);
      acc03 = PerBlock2x2MatMulMaybeEmulate(d_acc, A0, B3, acc03);
    }
    if constexpr (kNumQueries >= 3) {
      const auto A1 =
          LoadAndDuplicateQueries(d_input, q_group + qkv_dim * 2 + ch_base_q);
      acc10 = PerBlock2x2MatMulMaybeEmulate(d_acc, A1, B0, acc10);
      acc11 = PerBlock2x2MatMulMaybeEmulate(d_acc, A1, B1, acc11);
      acc12 = PerBlock2x2MatMulMaybeEmulate(d_acc, A1, B2, acc12);
      acc13 = PerBlock2x2MatMulMaybeEmulate(d_acc, A1, B3, acc13);
    }
    if constexpr (kNumQueries >= 5) {
      const auto A2 =
          LoadAndDuplicateQueries(d_input, q_group + qkv_dim * 4 + ch_base_q);
      acc20 = PerBlock2x2MatMulMaybeEmulate(d_acc, A2, B0, acc20);
      acc21 = PerBlock2x2MatMulMaybeEmulate(d_acc, A2, B1, acc21);
      acc22 = PerBlock2x2MatMulMaybeEmulate(d_acc, A2, B2, acc22);
      acc23 = PerBlock2x2MatMulMaybeEmulate(d_acc, A2, B3, acc23);
    }
    if constexpr (kNumQueries >= 7) {
      const auto A3 =
          LoadAndDuplicateQueries(d_input, q_group + qkv_dim * 6 + ch_base_q);
      acc30 = PerBlock2x2MatMulMaybeEmulate(d_acc, A3, B0, acc30);
      acc31 = PerBlock2x2MatMulMaybeEmulate(d_acc, A3, B1, acc31);
      acc32 = PerBlock2x2MatMulMaybeEmulate(d_acc, A3, B2, acc32);
      acc33 = PerBlock2x2MatMulMaybeEmulate(d_acc, A3, B3, acc33);
    }
    ch_base_k += ch_step * 32;  // Stride 128 elements for 8x4 layout
    ch_base_q += ch_step * 2;
  }

  auto convert_and_reduce = [&](VF& C, VecAcc acc) HWY_ATTR {
...
template <int kNumQueries, size_t kStepSize,
...
HWY_INLINE void TileFlashAttentionSVBlock(
  ...
  constexpr size_t kBlockSize = BENCHMARK_BLOCK_SIZE;

  // Pre-pack Q into BF16 to avoid scaling and demoting in the inner loop.
  const size_t kNumPackedVectors = ((kNumQueries + 1) / 2) * 2;
  HWY_ALIGN BF16 q_packed[kBlockSize * 8 * hn::MaxLanes(dbf)];

  for (size_t step_idx = 0; step_idx < actual_steps; ++step_idx) {
    auto load_scaled_q = [&](size_t qp, VF& v0, VF& v1) HWY_ATTR {
      const float* ptr =
          softmax_buf + (q_base_idx + qp) * kBlockSize + step_idx * kStepSize;
    ...
    auto pack_and_store_pair = [&](size_t pair_idx, VF ql0, VF qh0, VF ql1,
                                   VF qh1) HWY_ATTR {
      BF16* dst = q_packed + step_idx * kNumPackedVectors * kStepSize +
                  pair_idx * 2 * kStepSize;
      ...
      hn::StoreU(A1, dbf, dst + kStepSize);
    };
  ...
  // Pre-compute V tile pointers to avoid row lookups in the inner loop.
  const BF16* v_ptrs[8];
  ...
  size_t num_tiles_in_block =
      (actual_steps * kStepSize + KVCache::kTileSize - 1) / KVCache::kTileSize;
  for (size_t t = 0; t < num_tiles_in_block; ++t) {
    const BF16* tile_base = reinterpret_cast<const BF16*>(
        kvs[current_kv_idx].RowBytes(start_tile_idx + t));
    v_ptrs[t] = tile_base + qkv_dim * 32;
  }

  // Step-Dependent Pre-computation
  const BF16* step_q_ptrs[BENCHMARK_BLOCK_SIZE / 8];
  const BF16* step_v_tiles[BENCHMARK_BLOCK_SIZE / 8];
  size_t step_offsets_even[BENCHMARK_BLOCK_SIZE / 8];
  size_t step_offsets_odd[BENCHMARK_BLOCK_SIZE / 8];

  for (size_t step_idx = 0; step_idx < actual_steps; ++step_idx) {
    step_q_ptrs[step_idx] = q_packed + step_idx * kNumPackedVectors * kStepSize;
    size_t step_pos = position + step_idx * kStepSize;
    size_t global_token_pos = step_pos - current_kv_start_offset;
    size_t tile_idx = global_token_pos / 32 - start_tile_idx;
    step_v_tiles[step_idx] = v_ptrs[tile_idx];
    size_t t = step_pos % 32;
    size_t t_odd = t + kStepSize / 2;
  ...
  for (size_t ch_base = 0; ch_base < qkv_dim; ch_base += 8) {
    ...
    for (size_t step_idx = 0; step_idx < actual_steps; ++step_idx) {
      ...
      if constexpr (kNumQueries >= 1) {
        const auto A0 = hn::LoadU(dbf, q_ptr + 0);
        C00 = PerBlock2x2MatMulMaybeEmulate(df, A0, B_even0, C00);
        C01 = PerBlock2x2MatMulMaybeEmulate(df, A0, B_even1, C01);
        C02 = PerBlock2x2MatMulMaybeEmulate(df, A0, B_even2, C02);
        C03 = PerBlock2x2MatMulMaybeEmulate(df, A0, B_even3, C03);
      }
      if constexpr (kNumQueries >= 3) {
        const auto A2 = hn::LoadU(dbf, q_ptr + 2 * kStepSize);
        C10 = PerBlock2x2MatMulMaybeEmulate(df, A2, B_even0, C10);
        C11 = PerBlock2x2MatMulMaybeEmulate(df, A2, B_even1, C11);
        C12 = PerBlock2x2MatMulMaybeEmulate(df, A2, B_even2, C12);
        C13 = PerBlock2x2MatMulMaybeEmulate(df, A2, B_even3, C13);
      }
      if constexpr (kNumQueries >= 5) {
        const auto A4 = hn::LoadU(dbf, q_ptr + 4 * kStepSize);
        C20 = PerBlock2x2MatMulMaybeEmulate(df, A4, B_even0, C20);
        C21 = PerBlock2x2MatMulMaybeEmulate(df, A4, B_even1, C21);
        C22 = PerBlock2x2MatMulMaybeEmulate(df, A4, B_even2, C22);
        C23 = PerBlock2x2MatMulMaybeEmulate(df, A4, B_even3, C23);
      }
      if constexpr (kNumQueries >= 7) {
        const auto A6 = hn::LoadU(dbf, q_ptr + 6 * kStepSize);
        C30 = PerBlock2x2MatMulMaybeEmulate(df, A6, B_even0, C30);
        C31 = PerBlock2x2MatMulMaybeEmulate(df, A6, B_even1, C31);
        C32 = PerBlock2x2MatMulMaybeEmulate(df, A6, B_even2, C32);
        C33 = PerBlock2x2MatMulMaybeEmulate(df, A6, B_even3, C33);
      }

      // Odd halves second (A1, A3, A5, A7)
      if constexpr (kNumQueries >= 1) {
        const auto A1 = hn::LoadU(dbf, q_ptr + kStepSize);
        C00 = PerBlock2x2MatMulMaybeEmulate(df, A1, B_odd0, C00);
        C01 = PerBlock2x2MatMulMaybeEmulate(df, A1, B_odd1, C01);
        C02 = PerBlock2x2MatMulMaybeEmulate(df, A1, B_odd2, C02);
        C03 = PerBlock2x2MatMulMaybeEmulate(df, A1, B_odd3, C03);
      }
      if constexpr (kNumQueries >= 3) {
        const auto A3 = hn::LoadU(dbf, q_ptr + 3 * kStepSize);
        C10 = PerBlock2x2MatMulMaybeEmulate(df, A3, B_odd0, C10);
        C11 = PerBlock2x2MatMulMaybeEmulate(df, A3, B_odd1, C11);
        C12 = PerBlock2x2MatMulMaybeEmulate(df, A3, B_odd2, C12);
        C13 = PerBlock2x2MatMulMaybeEmulate(df, A3, B_odd3, C13);
      }
      if constexpr (kNumQueries >= 5) {
        const auto A5 = hn::LoadU(dbf, q_ptr + 5 * kStepSize);
        C20 = PerBlock2x2MatMulMaybeEmulate(df, A5, B_odd0, C20);
        C21 = PerBlock2x2MatMulMaybeEmulate(df, A5, B_odd1, C21);
        C22 = PerBlock2x2MatMulMaybeEmulate(df, A5, B_odd2, C22);
        C23 = PerBlock2x2MatMulMaybeEmulate(df, A5, B_odd3, C23);
      }
      if constexpr (kNumQueries >= 7) {
        const auto A7 = hn::LoadU(dbf, q_ptr + 7 * kStepSize);
        C30 = PerBlock2x2MatMulMaybeEmulate(df, A7, B_odd0, C30);
        C31 = PerBlock2x2MatMulMaybeEmulate(df, A7, B_odd1, C31);
        C32 = PerBlock2x2MatMulMaybeEmulate(df, A7, B_odd2, C32);
        C33 = PerBlock2x2MatMulMaybeEmulate(df, A7, B_odd3, C33);
      }
    }
...
HWY_ATTR void TileFlashAttentionReturnExpSumsAndMaxLogitsBF16_Macro(
  ...
  const DU du;
  using VF = hn::Vec<DF>;
  const size_t step_size = hn::Lanes(dbf);
  if (step_size > 32) {
    HWY_ABORT(
        "Unsupported step size (vector width) %zu. Only up to 512-bit (32 "
        "lanes) is supported.",
        step_size);
  }
  ...
  const size_t kStepsPerTile =
      std::max(size_t(1), KVCache::kTileSize / step_size);

  TileAttentionGroupParams preamble(q_count, kNumQueriesPerLoop, step_size,
  ...
  while (position <= largest_last_pos) {
    ...
    size_t actual_block_size =
        std::min(kBlockSize, hwy::RoundUpTo(remaining_tokens, step_size));
    actual_block_size = std::min(actual_block_size, kv_remaining);

    size_t actual_steps = actual_block_size / step_size;
    [[maybe_unused]] size_t actual_M =
        hwy::DivCeil(actual_block_size, kTileSize);
    ...
    auto inner_loop_qk = [&]<int kNumQueries>(size_t query_idx,
      ...
      if (step_pos + step_size <= min_start_pos_per_group[loop_idx] ||
        ...
        float* softmax_buf_ptr =
            softmax_buf.data() + query_idx * kBlockSize + step_idx * step_size;
        for (int q = 0; q < kNumQueries; ++q) {
          for (int t = 0; t < step_size; ++t) {
      ...
      const size_t current_pos = macro_tile_start_pos + step_idx * step_size;
      const BF16* tile_base =
          reinterpret_cast<const BF16*>(kvs[current_kv_idx].RowBytes(s_idx));

      const BF16* q_group = q_base + query_idx * qkv_dim;
      ...
      VF C00, C01, C02, C03, C10, C11, C12, C13, C20, C21, C22, C23, C30, C31,
          C32, C33;
      QDotKTilexUpTo8MatrixAccumulation<kNumQueries>(
          df, q_group, tile_base, current_pos, qkv_dim, C00, C01, C02, C03, C10,
          C11, C12, C13, C20, C21, C22, C23, C30, C31, C32, C33);
      ...
      if constexpr (kNumQueries >= 1) pack_queries(C00, C01, x_0_p_0, x_1_p_0);
      if constexpr (kNumQueries >= 1) pack_queries(C02, C03, x_0_p_1, x_1_p_1);
      if constexpr (kNumQueries >= 3) pack_queries(C10, C11, x_2_p_0, x_3_p_0);
      if constexpr (kNumQueries >= 3) pack_queries(C12, C13, x_2_p_1, x_3_p_1);
      if constexpr (kNumQueries >= 5) pack_queries(C20, C21, x_4_p_0, x_5_p_0);
      if constexpr (kNumQueries >= 5) pack_queries(C22, C23, x_4_p_1, x_5_p_1);
      if constexpr (kNumQueries >= 7) pack_queries(C30, C31, x_6_p_0, x_7_p_0);
      if constexpr (kNumQueries >= 7) pack_queries(C32, C33, x_6_p_1, x_7_p_1);
      ...
      if (current_pos < max_start_pos_per_group[loop_idx] ||
          current_pos + step_size - 1 > min_last_pos_per_group[loop_idx]) {
    ...
    for (size_t q_base_idx = 0; q_base_idx < q_count; q_base_idx += 8) {
      ...
      for (size_t q_offset = 0; q_offset < actual_q_count; ++q_offset) {
        ...
        if (position + actual_block_size <= start_pos_per_query[q] ||
          ...
          // Skip update for completely masked query in this block.
          // scales_old[q_offset] remains 1.0f, q_scales_new[q_offset] remains
          // 0.0f.
          continue;
        }
      ...
      auto call_sv_block = [&]<int kNumQueries>() HWY_ATTR {
        HWY_LANES_CONSTEXPR size_t step_size = hn::Lanes(dbf);
        if constexpr (HWY_HAVE_CONSTEXPR_LANES) {
          TileFlashAttentionSVBlock<kNumQueries, step_size, DF, DBF>(
              q_base_idx, position, current_kv_start_offset, current_kv_idx,
              actual_steps, qkv_dim, scales_old, q_scales_new,
              softmax_buf.data(), kvs, C_accumulators.data());
        } else {
          if (step_size == 32) {
            TileFlashAttentionSVBlock<kNumQueries, 32, DF, DBF>(
                q_base_idx, position, current_kv_start_offset, current_kv_idx,
                actual_steps, qkv_dim, scales_old, q_scales_new,
                softmax_buf.data(), kvs, C_accumulators.data());
          } else if (step_size == 16) {
            TileFlashAttentionSVBlock<kNumQueries, 16, DF, DBF>(
                q_base_idx, position, current_kv_start_offset, current_kv_idx,
                actual_steps, qkv_dim, scales_old, q_scales_new,
                softmax_buf.data(), kvs, C_accumulators.data());
          } else {  // step_size == 8 (guaranteed by top validation)
            TileFlashAttentionSVBlock<kNumQueries, 8, DF, DBF>(
                q_base_idx, position, current_kv_start_offset, current_kv_idx,
                actual_steps, qkv_dim, scales_old, q_scales_new,
                softmax_buf.data(), kvs, C_accumulators.data());
          }
