...
TEST(KVCacheTest, SharedLayersReserveNoCache) {
  ...
  ASSERT_TRUE(cache.HasLayerCaches());
  EXPECT_EQ(cache.Row(15, 0), cache.Row(13, 0));
  EXPECT_EQ(cache.LayerK(15).Row(0), cache.LayerK(13).Row(0));
}
...
TEST(KVCacheTest, WrapGrowthAndIndependentSnapshot) {
  ...
  inference.seq_len = 8192;
  RuntimeConfig runtime{};
  runtime.prefill_tbatch_size = 1;
  ThreadingContext ctx{ThreadingArgs{}};
  KVCache cache(config, inference, runtime, ctx.allocator);
  // Model the real layout after SIMD-specific transpose.
  constexpr size_t tile = 16;
  cache.LayerK(0).ReshapePackedRowsToCols(tile);
  cache.LayerV(0).ReshapePackedRowsToCols(tile);
  ...
  for (size_t p = 0; p < pos; ++p) {
    ...
    cache.Row(0, p)[0] = value;
    auto& k = cache.LayerK(0);
    auto& v = cache.LayerV(0);
    k.Row((p / tile) % k.Rows())[p % tile] = value;
    v.Row((p / tile) % v.Rows())[p % tile] = value;
  }
  auto snapshot = cache.Copy();
  cache.PrepareLayer(0, 1024, pos);
  EXPECT_GT(cache.LayerCapacity(0), original_rows);
  EXPECT_EQ(snapshot.LayerCapacity(0), original_rows);
  EXPECT_NE(cache.Row(0, pos - 1), snapshot.Row(0, pos - 1));
  for (size_t p = pos - 511; p < pos; ++p) {
    const float value = float(p % 128);
    EXPECT_EQ(hwy::ConvertScalarTo<float>(cache.Row(0, p)[0]), value);
    EXPECT_EQ(hwy::ConvertScalarTo<float>(snapshot.Row(0, p)[0]), value);
    auto& k = cache.LayerK(0);
    auto& v = cache.LayerV(0);
    EXPECT_EQ(
  ...
  cache.Clear();
  EXPECT_EQ(hwy::ConvertScalarTo<float>(cache.Row(0, pos - 1)[0]), 0.0f);
  EXPECT_EQ(hwy::ConvertScalarTo<float>(snapshot.Row(0, pos - 1)[0]),
            float((pos - 1) % 128));
}
...
TEST(KVCacheTest, FlashRingMatchesFullCache) {
  ...
  for (int64_t target : targets) {
    ...
    for (size_t queries : {1, 4, 8}) {
      ...
      runtime.prefill_tbatch_size = 256;
      KVCache cache(config, inference, runtime, ctx.allocator);
      auto& ring_k = cache.LayerK(0);
      auto& ring_v = cache.LayerV(0);
      ring_k.ReshapePackedRowsToCols(tile);
      ring_v.ReshapePackedRowsToCols(tile);
