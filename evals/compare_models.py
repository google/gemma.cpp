...
def parse_model_spec(data: dict[str, Any], base: Path) -> ModelSpec:
    ...
    try:
        name = data["name"]
        if not isinstance(data["weights"], str) or not data["weights"]:
            raise ValueError("weights must be a nonempty path string")
        weights = _resolve_path(data["weights"], base)
    ...
    if any(type(arg) not in (str, int, float) for arg in raw_args):
        raise ValueError(f"{name}: args must contain strings or numbers")
    ...
    if any(arg.split("=", 1)[0] in owned for arg in args):
        raise ValueError(f"{name}: args override a runner-owned option")
    ...
    if any(
        ...
        raise ValueError(f"{name}: invalid environment override")
...
def run_command(
    ...
    with stdout_path.open("w", encoding="utf-8") as stdout, stderr_path.open(
        ...
        try:
            ...
            # Interrupting the runner must not leave model processes running.
    ...
    if return_code != 0:
        tail = "\n".join(
            stderr_path.read_text(encoding="utf-8", errors="replace").splitlines()[-20:]
        )
        raise RuntimeError(
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        if line.startswith(prefix):
            if found is not None:
                raise ValueError(f"{path}: duplicate {prefix.strip()} line")
            found = json.loads(line[len(prefix) :])
            if not isinstance(found, dict):
                raise ValueError(f"{path}: {prefix.strip()} must be an object")
    if found is None:
...
def parse_entropy(path: Path) -> dict[str, float | int]:
    ...
    speed_matches = re.findall(r"\[([0-9.eE+-]+) tokens / sec\]", text)
    ...
    if (
        ...
        raise ValueError(f"{path}: invalid cross-entropy metrics")
    return {
        "tokens": tokens,
...
def render_table(rows: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "MMLU accuracy | Flips | Mean KL | p95 KL | Peak RSS | Wall s | Generate s* | Generate speedup |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        ...
        if entropy and root_entropy:
            entropy_delta = (
                100.0
                * (entropy["bits_per_token"] / root_entropy["bits_per_token"] - 1.0)
                if root_entropy["bits_per_token"]
                else None
            )
            speedup = 100.0 * (
                entropy["tokens_per_second"] / root_entropy["tokens_per_second"] - 1.0
            )
            entropy_text = f"{entropy['bits_per_token']:.4f}"
            delta_text = "—" if entropy_delta is None else f"{entropy_delta:+.3f}%"
        ...
        lines.append(
            ...
            "{wall:.3f} | {inference} | {infer_speedup} |".format(
                wall=row["mmlu_wall_seconds"],
    ...
    return "\n".join(lines) + (
        "\n\n*Generate time excludes the evaluation sampling callback; includes "
        "MatMul autotuning. Wall time includes startup and reference I/O.\n"
        "Timing-dependent MatMul schedules can change outputs between runs, "
        "including self-comparisons.\n"
        "Flips count changes in correctness; wrong-to-wrong answer changes "
        "are reported separately in JSON. KL is full-vocabulary root || target "
        "at the first answer token, in nats. Peak RSS is sampled on Linux "
        "and may miss short-lived peaks.\n"
    )
...
def run_evaluation(
    ...
    mmlu_out = output_dir / f"{spec.name}.mmlu.out"
    mmlu_err = output_dir / f"{spec.name}.mmlu.err"
    command = [
    ...
    command.extend(["--reference_out" if is_root else "--reference_in", str(reference)])
    ...
    results = load_results(mmlu_out)
    correct = sum(row["correct"] for row in results.values())
    if (
        mmlu_summary.get("answers") != len(results)
        or mmlu_summary.get("correct") != correct
        or not math.isclose(mmlu_summary.get("accuracy", -1), correct / len(results))
    ):
        raise ValueError(f"{mmlu_out}: summary does not match question results")
    timing = parse_prefixed_json(mmlu_out, "MMLU_TIMING ")
    for key in ("generate_seconds", "sample_seconds", "inference_seconds"):
        value = timing[key]
        if type(value) not in (int, float) or not math.isfinite(value) or value < 0:
            raise ValueError(f"{mmlu_out}: invalid {key}")
    kl_summary = None if is_root else parse_prefixed_json(mmlu_out, "MMLU_KL_SUMMARY ")

    if kl_summary is not None:
        if (
            kl_summary.get("samples") != len(results)
            or kl_summary.get("direction") != "root||target"
            or kl_summary.get("unit") != "nats"
        ):
            raise ValueError(f"{mmlu_out}: invalid KL summary")
        for value in [kl_summary[key] for key in ("mean", "median", "p95", "max")] + [
            row["full_vocab_kl"] for row in results.values()
        ]:
            if type(value) not in (int, float) or not math.isfinite(value) or value < 0:
                raise ValueError(f"{mmlu_out}: invalid KL value")

    entropy: dict[str, float | int] | None = None
    entropy_run: RunMetrics | None = None
    if entropy_path is not None:
        entropy_out = output_dir / f"{spec.name}.entropy.out"
        entropy_err = output_dir / f"{spec.name}.entropy.err"
        entropy_command = [
        ...
        entropy_run = run_command(entropy_command, spec.env, entropy_out, entropy_err)
    ...
    return (
        {
            ...
            "entropy_wall_seconds": (
                None if entropy_run is None else entropy_run.wall_seconds
            ),
...
def main() -> int:
    ...
    try:
        ...
        if output_dir.exists() and any(output_dir.iterdir()):
            raise ValueError("output directory must be empty; choose a new output_dir")
        output_dir.mkdir(parents=True, exist_ok=True)
        reference = output_dir / f"{root.name}.root-kl.jsonl"
        ...
        for target in targets:
            ...
            row["flips"] = compare_results(root_results, load_results(target_output))
            rows.append(row)
...
if __name__ == "__main__":
    try:
        ...
        print("error: comparison interrupted", file=sys.stderr)
