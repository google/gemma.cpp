...
static bool MakeBrgemm(dnnl::ukernel::brgemm& brg, int64_t m, int64_t n,
  ...
  try {
    brg = dnnl::ukernel::brgemm(m, n, k, batch, lda, ldb, ldc, a_dt, b_dt, c_dt,
                                true);
    if (!brg) {
...
static HWY_NOINLINE bool InitBRGeMMKernels(const BRGeMMConfig& cfg, size_t M,
  ...
  const int64_t ldb_for[2] = {
      static_cast<int64_t>(ke.N_blk),
      static_cast<int64_t>(ke.N_tail ? ke.N_tail : ke.N_blk)};
  const int64_t ldc_for[2] = {
      static_cast<int64_t>(ke.N_blk),
      static_cast<int64_t>(ke.N_tail ? ke.N_tail : ke.N_blk)};
    ...
    for (int ni = 0; ni < 2; ++ni) {
      ...
      if (ke.K_chunks > 0) {
        if (!MakeBrgemm(ke.brg_first_all[mi][ni], ms, ns,
                        ...
                        static_cast<int64_t>(ke.lda), ldb_for[ni], ldc_for[ni],
                        a_dt, b_dt, c_dt, false)) {
          return false;
        }
        max_sp =
            std::max(max_sp, ke.brg_first_all[mi][ni].get_scratchpad_size());
      }
      if (ke.K_super_blocks > 1) {
        ...
        max_sp = std::max(max_sp, ke.brg_full[mi][ni].get_scratchpad_size());
      }
      if (ke.K_super_rem > 0) {
        const bool rem_is_first = (ke.K_super_blocks == 0);
        auto& target =
            rem_is_first ? ke.brg_first_rem[mi][ni] : ke.brg_rem[mi][ni];
        if (!MakeBrgemm(target, ms, ns, static_cast<int64_t>(ke.K_blk),
                        static_cast<int64_t>(ke.batch_rem),
                        static_cast<int64_t>(ke.lda), ldb_for[ni], ldc_for[ni],
                        a_dt, b_dt, c_dt, !rem_is_first)) {
          return false;
        }
      ...
      if (ke.K_tail > 0) {
        ...
        if (!MakeBrgemm(ke.brg_ktail[mi][ni], ms, ns,
                        ...
                        static_cast<int64_t>(ke.lda), ldb_for[ni], ldc_for[ni],
                        a_dt, b_dt, c_dt, add_c)) {
          return false;
        }
        max_sp = std::max(max_sp, ke.brg_ktail[mi][ni].get_scratchpad_size());
      }
    }
  }
  ...
  if (ke.need_pack) {
    for (int ni = 0; ni < 2; ++ni) {
      ...
      const int64_t ns = static_cast<int64_t>(ke.n_sizes[ni]);
      if (ke.K_chunks > 0) {
        const int64_t K_full = static_cast<int64_t>(ke.K_chunks * ke.K_blk);
        try {
          ...
          ke.blocked_B_size[ni] =
              static_cast<size_t>(ldb_for[ni]) * ke.K_blk * ke.b_dt_size;
        } catch (...) {
          return false;
        }
      }
  ...
  for (int ni = 0; ni < 2; ++ni) {
    ...
    if (ke.K_chunks > 0) {
      ...
      for (size_t i = 0; i < ke.K_super_size; ++i) {
        const int64_t a_off = static_cast<int64_t>(i * ke.K_blk * ke.a_dt_size);
        const int64_t b_off =
            ke.need_pack
    ...
    if (ke.K_super_blocks > 1) {
      ...
      for (size_t ks = 1; ks < ke.K_super_blocks; ++ks) {
        ...
        for (size_t i = 0; i < ke.batch_full; ++i) {
          ...
          const int64_t b_off =
              ke.need_pack ? static_cast<int64_t>(k_idx * ke.blocked_B_size[ni])
      ...
      for (size_t i = 0; i < ke.K_super_rem; ++i) {
        ...
        const int64_t b_off =
            ke.need_pack
                ...
                : static_cast<int64_t>(k_idx * cur_n * ke.K_blk * ke.b_dt_size);
...
static HWY_NOINLINE bool DoMatMul_BRGeMM(
  ...
  const BRGeMMKernelKey kern_key{
      M, K, N, cfg.M_blk, cfg.N_blk, cfg.K_blk, cfg.batch_size};
  ...
  if (pb_it == pb_cache.end()) {
    ...
    if (ke.need_pack) {
      ...
      for (size_t nt = 0; nt < ke.N_total_tiles; ++nt) {
        const int ni = (nt < ke.N_full_tiles) ? 0 : 1;
        const size_t b_row =
            (nt < ke.N_full_tiles) ? nt * ke.N_blk : ke.N_full_tiles * ke.N_blk;
        const uint8_t* B_in = B_base + b_row * ke.ldb_orig * ke.b_dt_size;
  ...
  const uint8_t* B_packed = ke.need_pack ? pe.B_packed_buf.data() : nullptr;
  ...
  const auto execute_tile = [&](size_t m_start, size_t n_start, size_t k_super,
                                float* temp_C, uint8_t* scratch) HWY_ATTR {
    ...
    const void* B_tile =
        ke.need_pack
            ? static_cast<const void*>(B_packed + pe.B_tile_offset[n_tile_idx])
            : static_cast<const void*>(B_base +
                                       real_n * ke.ldb_orig * ke.b_dt_size);

    float* C_tile_ptr = temp_C;
    const size_t k_total = ke.K_super_blocks + (ke.K_super_rem > 0 ? 1 : 0);
    ...
    if (is_last) {
      if (ke.K_tail > 0) {
        const uint8_t* A_ktail = A_tile + ke.K_chunks * ke.K_blk * ke.a_dt_size;
        const void* B_ktail =
            ke.need_pack
                ? static_cast<const void*>(B_packed +
                                           pe.B_ktail_offset[n_tile_idx])
                : static_cast<const void*>(
                      B_base + (real_n * ke.ldb_orig + ke.K_chunks * ke.K_blk) *
                                   ke.b_dt_size);
  ...
  const hwy::pool::Caller caller = ctx.pool_callers.Get(Callers::kBRGeMM);
  ...
  ParallelForWithinCluster(
      ...
      [&](uint64_t task_idx, size_t /*worker*/) HWY_ATTR {
        ...
        const size_t n_begin = task_idx * tiles_per_task +
        ...
        for (size_t ks = 0; ks < k_iters; ++ks) {
          ...
          for (size_t nt = n_begin; nt < n_end; ++nt) {
            ...
            for (size_t mt = 0; mt < total_m_tiles; ++mt) {
              ...
              float* temp_C = tc_base + (mt * n_tiles_in_range + n_idx) *
