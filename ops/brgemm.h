...
// Enabled at compile time via GEMMA_ONEDNN_BRGEMM=1 (Bazel: --define
...
// opt-in
#ifndef GEMMA_ONEDNN_BRGEMM
#define GEMMA_ONEDNN_BRGEMM 0
...
inline std::vector<BRGeMMConfig> BRGeMMCandidates(size_t M, size_t K,
  ...
  for (size_t mb : kMBlkValues) {
    ...
    for (size_t bs : kBatchValues) {
      const size_t eff_bs = (k_chunks > 0) ? std::min(bs, k_chunks) : size_t{1};
      bool dup = false;
