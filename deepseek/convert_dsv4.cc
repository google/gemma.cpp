...
#define HWY_TARGET_INCLUDE "deepseek/convert_dsv4.cc"  // NOLINT
  ...
  ShardedCheckpoint(const std::string& dir, bool tolerate_missing,
    ...
    for (const auto& [tensor, file] : index["weight_map"].items()) {
      ...
      if (file_indices.insert({file_str, files_.size()}).second) {
        std::unique_ptr<File> f = OpenFileOrNull(Path(dir + "/" + file_str),
                                                 "r");
...
static const float kFP4Table[8] = {0.0f, 0.5f, 1.0f, 1.5f, 2.0f, 3.0f,
                                   4.0f, 6.0f};
 ...
 public:
  ...
  void Run() {
    ...
    weights.ForEachTensor(nullptr, nullptr, [&](const TensorArgs& targs) {
      ...
      if (base == "gating_ein" || base == "skip_scale" ||
          base == "qkv_ein" || base == "qkv1_w" || base == "qkv2_w" ||
          base == "att_w" || base == "att_ein") {
        return;
      }
