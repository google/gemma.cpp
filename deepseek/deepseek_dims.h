...
struct MLADims {
  explicit MLADims(const ModelConfig& config) {
    for (const LayerConfig& lc : config.layer_configs) {
      ...
      kv_a_dim = HWY_MAX(kv_a_dim, size_t{lc.kv_lora_rank} + lc.rope_head_dim);
      indexer_dim =
          HWY_MAX(indexer_dim, size_t{lc.indexer_heads} * lc.indexer_head_dim);
      rope_head_dim = HWY_MAX(rope_head_dim, size_t{lc.rope_head_dim});
      if (lc.HasCompressor()) {
        comp_dim = HWY_MAX(comp_dim, lc.CompressorCoff() * lc.KVLatentDim());
        if (lc.HasIndexer()) {
          idxc_dim = HWY_MAX(idxc_dim,
                             lc.CompressorCoff() * size_t{lc.indexer_head_dim});
        }
      }
      if (lc.IsV4MLA()) {
        o_in_dim =
            HWY_MAX(o_in_dim, size_t{lc.heads} / lc.o_groups * lc.qkv_dim);
        o_mid_dim = HWY_MAX(o_mid_dim, size_t{lc.o_groups} * lc.o_lora_rank);
      }
    }
  }
...
static inline MatStorageT<float> CreateYarnInvTimescale(
  ...
  MatStorageT<float> inv_timescale("inv_timescale_c", total_entries, allocator);
