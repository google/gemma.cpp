...
static void HCSigmoidRead(const float* HWY_RESTRICT mixes,
                          ...
                          float eps, size_t hc_mult, const float* HWY_RESTRICT s,
                          float* HWY_RESTRICT x, size_t model_dim,
                          ThreadingContext& ctx, size_t worker) {
      ...
      [&](size_t token_idx, size_t worker) HWY_ATTR {
        ...
        const float rsqrt_ms = detail::RMSNormMul(s, flat, env.ctx, worker);
        {
          ...
          for (size_t i = 0; i < mix_hc; i += NH) {
            ...
            hn::StoreN(hn::Mul(hn::LoadN(dh, mixes + i, n), vr), dh,
                       mixes + i, n);
          }
        }
        ...
        for (size_t i = 0; i < hc_mult; ++i) {
          ...
          for (size_t j = 0; j < hc_mult; j += NH) {
            const size_t n = HWY_MIN(NH, hc_mult - j);
            const VH e = hn::Exp(dh, hn::Sub(hn::LoadNOr(vneg, dh, row + j, n), vm));
        ...
        for (size_t iter = 0;; ++iter) {
          ...
          for (size_t i = 0; i < hc_mult; ++i) {  // rows
            ...
            for (size_t j = 0; j < hc_mult; j += NH) {
              sum += hn::ReduceSum(dh,
                                   hn::LoadN(dh, row + j, HWY_MIN(NH, hc_mult - j)));
            }
...
static HWY_NOINLINE void HCWriteDynamic(const MatPtrT<T>& block_out,
  ...
  ParallelFor(
      ...
      [&](size_t token_idx, size_t worker) HWY_ATTR {
        ...
        const float* HWY_RESTRICT post =
            activations.hc_post_w.Row(token_idx);
...
static bool CompressorStep(const float* HWY_RESTRICT kv_row,
  ...
  memcpy(kv_dst, kv_row, width * sizeof(float));
  {
    ...
    size_t i = 0;
    for (; i + NF <= width; i += NF) {
      hn::StoreU(hn::Add(hn::LoadU(df, gate_row + i), hn::LoadU(df, ape_row + i)),
                 df, score_dst + i);
    }
  ...
  for (; k + NF <= d; k += NF) {
    ...
    for (size_t r = 0; r < num_rows; ++r) {
      ...
      vden = hn::Add(vden, w);
      vacc = hn::MulAdd(w, hn::LoadU(df, state.kv_state + row_ofs(r, k)),
                        vacc);
    }
  ...
  ParallelFor(
      ...
      Callers::kAttComputeQKV, [&](size_t task, size_t worker) HWY_ATTR {
        ...
        const size_t token_idx =
            div_qbatch.Divide(static_cast<uint32_t>(task));
        const size_t cache_pos = qbatch.Pos(qi) + token_idx;
        ...
        for (size_t h = 0; h < idx_heads; ++h) {
          Rope(q + h * idx_dim + idx_dim - rope_dim, rope_dim,
               inv_timescale_c, static_cast<int>(cache_pos), env.ctx, worker);
        }
        ...
        for (size_t e0 = 0; e0 < num_sealed; e0 += 4) {
          ...
          for (size_t h = 0; h < idx_heads; ++h) {
            ...
            Dot4(q + h * idx_dim, k_f[0], k_f[1], k_f[2], k_f[3], idx_dim,
                 d4);
            const float wh = w[h];
        ...
        if (top_k < num_sealed) {
          std::partial_sort(selected[task].begin(),
                            ...
                            selected[task].end(),
                            [&](uint32_t a, uint32_t b) HWY_ATTR {
...
static HWY_NOINLINE void DeepSeekAttention(size_t num_tokens, size_t layer_idx,
  ...
  const size_t window =
      layer_idx < config.attention_window_sizes.size()
  ...
  ParallelFor(
      ...
      Callers::kAttComputeQKV, [&](size_t task, size_t worker) HWY_ATTR {
        ...
        const auto process_row = [&](size_t row, size_t offset) HWY_ATTR {
          ...
          DecompressAndZeroPad(df, MakeSpan(src, kv_a_dim), 0,
                               blk_vals[blk_n], kv_a_dim);
...
struct DeepSeekMoE {
  ...
  static HWY_NOINLINE void ChooseExperts(const LayerWeightsPtrs& layer,
    ...
    ParallelFor(
        ...
        Callers::kMoEChooseExperts, [&](size_t token_idx, size_t worker)
            HWY_ATTR {
              ...
              for (uint32_t e = 0; e < num_experts; ++e) {
                switch (scoring) {
                  ...
                  case RouterScoreFunc::kSqrtSoftplus: {
                    ...
                    const float sp = logits[e] > 20.0f
                                         ? logits[e]
                                         : log1pf(expf(logits[e]));
                    probs[e] = sqrtf(sp);
                    break;
                  }
                  default:
              ...
              if (hash) {
                const int32_t token_id =
                    token_idx < activations.token_ids.size()
                        ? activations.token_ids[token_idx]
                        : 0;
                HWY_ALIGN float row[64];
                ReadRowF32(layer.hash_tid2eid,
                           static_cast<size_t>(token_id), row,
                           experts_per_token);
  ...
  static HWY_NOINLINE void ClampSwiGLU(MatPtrT<BF16>& C1, MatPtrT<BF16>& C2,
    ...
    for (size_t r = 0; r < rows; ++r) {
      ...
      for (size_t c = 0; c < cols; ++c) {
        ...
        up[c] =
            hwy::ConvertScalarTo<BF16>(HWY_MAX(HWY_MIN(u, limit), -limit));
      }
    }
  }
...
void DeepSeekTransformerLayer(size_t num_tokens, size_t layer_idx,
  ...
  RMSNormBatched</*kPlainWeight=*/true>(activations.x,
...
void DeepSeekMTPStep(size_t num_tokens, const int* next_tokens,
  ...
  for (size_t r = 0; r < num_tokens; ++r) {
    ...
    for (size_t i = 0; i < hc_mult; ++i) {
      ScaledRMSNorm(weights.mtp_hnorm, h + i * model_dim, model_dim, env.ctx, 0);
    }
